...
class TestZeroInitEquivalence:
    """
    Core correctness test: with zero-init les_charge_readout (E_LR = 0), the
    LES model must produce energy, forces, and stress consistent with the
    original non-LES checkpoint.

    Note on tolerance: the LES-legacy model uses ForceStressOutput (positional
    gradient via EdgePreprocess), while the SR model uses ForceStressOutputFromEdge
    (edge virial).  These are mathematically equivalent but not bit-identical in
    float32, so atol is relaxed to 1e-4 for forces/stress.
    """
